...
function blockEntityLoot(args: BlockEntityData, id: string) {
    ...
    if (!ObjectUtil.isSamePos(args.entity.location, args.blockEntityDataLocation))
...
export class CookingPotBlockEntity extends BlockEntity {
    ...
    tick(args: any) {
        ...
        if (!recipeFactory.get(entity.id)) {
            cookingPotRecipe = new CookingPotRecipeHolder(entity, 6, 1, ["cooking_pot"], COOKING_POT_RECIPES);
            recipeFactory.set(entity.id, cookingPotRecipe);
        } else {
            cookingPotRecipe = recipeFactory.get(entity.id) as CookingPotRecipeHolder;
        }
