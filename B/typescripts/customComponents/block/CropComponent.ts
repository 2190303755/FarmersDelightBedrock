...
class TorchflowerComponent implements BlockCustomComponent {
    ...
    onPlayerInteract(args: BlockComponentPlayerInteractEvent): void {
        ...
        const random = Math.floor(Math.random() * 101);
        if (!player) return;
        if (container?.getSlot(player.selectedSlotIndex)?.typeId != "minecraft:bone_meal") return;
        try {
            if (age < 7) {
                if (player.getGameMode() == GameMode.Creative) {
                    dimension.spawnParticle("minecraft:crop_growth_emitter", block.center());
                    ...
                    if (player?.getGameMode() == GameMode.Creative) {
                        ...
                        if (topBlockId == "minecraft:air") {
                            ...
                            system.run(() => {
                                ...
                                growthBlock?.setPermutation(
                                    growthBlock.permutation.withState("farmersdelight:growth", 3)
                                );
                            });
                        }
                    } else {
        ...
        if (block.typeId == "farmersdelight:rice_block_upper") {
            try {
                ...
                if (growth == 3) {
                    ...
                    spawnLoot("farmersdelight/crops/farmersdelight_rice_riped", dimension, {
                        x: block.location.x,
                        y: block.location.y,
                        z: block.location.z,
                    });
